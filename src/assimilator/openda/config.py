...
def is_season_keyed(obs_std):
    """True if obs_std is {(depth, season): sigma} rather than a plain scalar."""
    return isinstance(obs_std, dict) and bool(obs_std) and all(isinstance(k, tuple) for k in obs_std)
...
def series_specs(depths, obs_std):
    ...
    its own standardDeviation. That is how the season pair reaches OpenDA EXACTLY -- the
    stochObserver carries one standardDeviation per series, and the series are ours to define
    (functions.sigma_obs_by_depth_season). A (depth, season) with no observations in the window is
    not emitted, so an operational window inside one season declares only that season's series."""
...
def simstrat_time(iso):
    """Exact fractional Simstrat day of an ISO date or datetime (naive = UTC).
    Same arithmetic as the adapter's observation times, so both give the same float."""
    dt = datetime.fromisoformat(re.sub(r"\.\d+", "", str(iso)))
    if dt.tzinfo is not None:
        dt = dt.astimezone(timezone.utc)
    minutes = dt.hour * 60 + dt.minute + dt.second / 60.0
    return (dt.date() - date(SIMSTRAT_REF_YEAR, 1, 1)).days + (minutes / 60.0) / 24.0


def run_window_days(start_date, end_date, exact=False):
    """(start_day, end_day) of the OpenDA run in Simstrat days.
    Normal run: whole days, end_day = end_date + 1 so the last day's observation is included.
    Restart cycle (exact=True): the two instants as given, the window (start, end]."""
    if exact:
        return simstrat_time(start_date), simstrat_time(end_date)
    return _simstrat_day(start_date), _simstrat_day(end_date) + 1
...
# Restart cycle only: read the previous cycle's ensemble, save this cycle's at the end.
# An empty restartInFile means a cold start (OpenDA ignores it).
_ODA_RESTART = """
\t<restartInFile>{in_file}</restartInFile>
\t<restartOutFilePrefix>rst_</restartOutFilePrefix>
\t<restartOutFileExtension>.zip</restartOutFileExtension>
\t<restartOutFileOnlyAtEndOfRun>true</restartOutFileOnlyAtEndOfRun>{seed}
"""

# Restart cycle only: this cycle's random seed (otherwise every cycle repeats the same draws).
_ODA_SEED = """
\t<initialSeed type="specify" seedValue="{seed}" />"""
...
# Restart declaration (PF, and any filter in a restart cycle — see render).  Both files travel together
...
\t<analysisTimes type="fromObservationTimes"{analysis_attrs} />
...
time: [{start_day}, 1.0, {end_day}]  #oda:time_control
"""


# All four row sets below are driven by series_specs, so the series a run declares are decided
# once. A disagreement between them is fatal inside OpenDA, with nothing in our log to explain it.

def _predictor_lines(depths, obs_std=DEFAULT_OBS_STD):
    return "\n".join(f'\t\t\t<vector id="{sid}" />' for sid, _, _ in series_specs(depths, obs_std))
...
def _output_lines(depths, obs_std=DEFAULT_OBS_STD):
    ...
    for sid, _, _ in specs:
        vid = f'"{sid}"'
        lines.append(f'\t\t<vector id={vid:<{pad}} ioObjectId="output" elementId="model.{sid}" />')
    return "\n".join(lines)


def _obs_formatter_rows(depths, obs_std):
    """How the fitted observation-error model reaches OpenDA: the stochObserver carries one
    standardDeviation per SERIES, so a season-keyed obs_std gives each season its own sigma and
    reproduces the native engine exactly."""
    return "\n".join(
...
def render(openda_dir, filter_type, n_members, obs_depths, start_date, end_date,
           obs_std=DEFAULT_OBS_STD, results_subdir="Results", max_threads=None, restart=None, seed=None):
    ...
    `restart` makes this a restart cycle: None (default) = normal run, unchanged; "" = cold start
    of a chain; a path = continue from that restart file. `seed` sets the cycle's random seed.
    """
    ...
    # Normal run: end_day = end_date + 1 (midnight after end_date) so the last day's observation
    # falls inside the window. Note: the native EnKF instead clamps its last window to end_date
    # midnight, so OpenDA may run one extra assimilation at the very end.
    # Restart cycle: exact instants, full precision. Never skipAtInitialTime: OpenDA then skips the
    # run's first analysis, which is the cycle's only one.
    cycle = restart is not None
    start_day, end_day = run_window_days(start_date, end_date, exact=cycle)
    if cycle:
        t_start, t_end = repr(float(start_day)), repr(float(end_day))
        tc_start, tc_end = t_start, t_end
        analysis_attrs = ' skipAtFinalTime="false"'
    else:
        t_start, t_end = start_day, end_day
        tc_start, tc_end = f"{start_day:.1f}", f"{end_day:.1f}"
        analysis_attrs = ""
    # Declare each member's state files (so OpenDA can save/restore them) for the PF, or when this
    # run hands its ensemble on to the next cycle.
    save_member_state = bool(spec.get("needs_restart")) or cycle
    # restartInFile must be relative to openda_dir: OpenDA prepends that folder even to absolute paths.
    in_file = os.path.relpath(restart, openda_dir).replace(os.sep, "/") if restart else ""
    ...
    oda = _ODA.format(
        ...
        restart="" if not cycle else _ODA_RESTART.format(
            in_file=xml_escape(in_file), seed="" if seed is None else _ODA_SEED.format(seed=int(seed))),
    )
    ...
    stochmodel = _STOCHMODEL.format(predictors=_predictor_lines(depths, obs_std),
                                    stoch_restart_info=_STOCH_RESTART_INFO if save_member_state else "")
    ...
    model      = _MODEL.format(filter=filter_type, instance_dir=instance_dir,
                               ...
                               restart_info=_RESTART_INFO if save_member_state else "")
    obs_fmt    = _OBS_FORMATTER.format(ref_year=SIMSTRAT_REF_YEAR, start_day=t_start,
                                       end_day=t_end, rows=_obs_formatter_rows(depths, obs_std))
    model_fmt  = _MODEL_FORMATTER.format(ref_year=SIMSTRAT_REF_YEAR, rows=_model_formatter_rows(depths, obs_std))
    time_ctrl  = _TIME_CONTROL.format(ref_year=SIMSTRAT_REF_YEAR, start_day=tc_start, end_day=tc_end)
